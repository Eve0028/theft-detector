...
try:
    ...
    from brainaccess.core import scan as ba_scan, init as ba_init, close as ba_close
    ...
    logger.error(f"BrainAccess SDK not available: {exc}")
...
class BrainAccessHandler:
    ...
    def annotate(self, marker: str) -> bool:
        ...
        """
        if not self.is_connected or self.eeg_manager is None:
            return False
        
        try:
            # Use native BrainAccess SDK annotation
    ...
    def _on_chunk(self, chunk_arrays, chunk_size: int) -> None:
        ...
        """
        # Fast path - no lock if not recording
    ...
    def _save_to_fif(self) -> bool:
        ...
        try:
            ...
            if self.eeg_manager:
                try:
                    sdk_annotations = self.eeg_manager.get_annotations()
                    ...
                    if sdk_annotations and len(sdk_annotations) > 0:
                        ...
                        for ann in sdk_annotations:
                            # ann is Annotation object from SDK
                            # Has .time (timestamp) and .annotation (string)
                        ...
                        if self.verbose:
                            self.logger.info(f"Embedded {len(sdk_annotations)} annotations")
                
                except Exception as e:
                    if self.verbose:
                        self.logger.warning(f"Could not retrieve annotations: {e}")
